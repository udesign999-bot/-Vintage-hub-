...
function buildLayerEl(layer) {
  ...
  if (layer.type === 'image') {
    ...
    rot.className = 'layer-rotatectl';
    const rotL = document.createElement('button'); rotL.textContent = '↺'; rotL.title = 'Rotate left';
    const rotR = document.createElement('button'); rotR.textContent = '↻'; rotR.title = 'Rotate right';
    rotL.addEventListener('click', (e) => { e.stopPropagation(); applyRotation(-15); });
    rotR.addEventListener('click', (e) => { e.stopPropagation(); applyRotation(15); });
    rot.appendChild(rotL); rot.appendChild(rotR);
    el.appendChild(rot);
  } else {
    ...
    rotL.addEventListener('click', (e) => { e.stopPropagation(); applyRotation(-15); });
    rotR.addEventListener('click', (e) => { e.stopPropagation(); applyRotation(15); });
    ctl.appendChild(rotL); ctl.appendChild(minus); ctl.appendChild(plus); ctl.appendChild(rotR);
    el.appendChild(ctl);
  }
...
function startDrag(e, layer, el) {
  if (e.target.closest('.layer-del') || e.target.closest('.layer-resize') || e.target.closest('.layer-fontctl') || e.target.closest('.layer-edit') || e.target.closest('.layer-rotatectl')) return;
  e.preventDefault();
  const stage = document.getElementById('designerStage');
...
document.getElementById('textConfirm').addEventListener('click', () => {
  ...
  currentLayers().push({ id: Date.now() + Math.random(), type: 'text', text, x: 25, y: 45, fontSize: 22, rotation: 0 });
